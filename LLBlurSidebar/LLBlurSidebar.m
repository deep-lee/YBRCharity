...
//    [self.view addGestureRecognizer:recognizer];
    ...
    // 列表
    CGRect rect = CGRectMake(kWindowWidth, 0, kWindowWidth, kWindowHeight);
...
#pragma mark - 显示/隐藏
- (BOOL)isSidebarShown{
    return self.contentView.frame.origin.x < kWindowWidth ? YES :NO;
}

- (void)showHideSidebar
{
    if (self.contentView.frame.origin.x == kWindowWidth) {
        LLLog(@"开始时在-点");
...
- (void)autoShowHideSidebar
{
    ...
    else
    {
        LLLog(@"自动缩回");
        [UIView animateWithDuration:duration animations:^{
            [self setSidebarOriginX:kWindowWidth];
        } completion:^(BOOL finished) {
            _isMoving = NO;
            self.view.hidden = YES;
        }];
